...
describe('Shared Variables For Testing Purposes:', () => {
  let username, id, password, date, trips, year, destinationData;
  beforeEach(() => {
    username = 'traveler50'; 
  ...
  describe('getUserID Tests:', () => {
    it('should return userID', () => {
      expect(getUserID(username)).to.equal(50);
    });
    it('should return userID for mulitple userIDs from 1-50', () => {
      const username1 = 'traveler1'; 
      expect(getUserID(username1)).to.equal(1);
    });
    it('should return message if username above 50 or invalid username entered', () => {
      const username1 = 'traveler200'
      const username2 = 'randomPassword'
      const username3 = 'traveler51'
      ...
      expect(getUserID(username3)).to.equal('Please enter a valid username');
    });
  });

  describe('handleLogin() Tests:', () => {
    it('should return true if valid username from 1-50 and password entered', () => {
      expect(handleLogin(username, password, id)).to.equal(true);
    });
    it('should return true for another valid username from 1-50 and password entered', () => {
      ...
      expect(handleLogin(anotherUsername, password, anotherId)).to.equal(true);
    });
    ...
    it('should return message when invalid username or password entered', () => {
      ...
      expect(handleLogin(invalidUsername, password, id)).to.equal('Invalid username and/or password. Please enter correct username and password.');
      expect(handleLogin(invalidUsername, invalidPassword, id)).to.equal('Invalid username and/or password. Please enter correct username and password.');
      expect(handleLogin(username, invalidPassword, id)).to.equal('Invalid username and/or password. Please enter correct username and password.');
    });
  });
  ...
  describe('getTodaysDate Tests:', () => {
    ...
    expect(getTodaysDate(currDate)).to.match(/^\d{4}\/\d{2}\/\d{2}$/)
    });
  });
  ...
  describe('Traveler Functions Tests:', () =>  {
    ...
    it('getSpecificTravelerTrips function should filter trips data by specific user login id that exists in trips data', () => {
      expect(getSpecificTravelerTrips(trips, id)).to.deep.equal([{"id": 196, "userID": 5, "destinationID": 16, "travelers": 1, "date": "2025/09/25", "duration": 8, "status": "pending", "suggestedActivities": []},
    ...
    it('travelerPastTrips function should filter the specific user trips data by the log in date and return all past trips from that date', () => {
      ...
      expect(travelerPastTrips(filteredTrips, date)).to.deep.equal([{"id": 91, "userID": 5, "destinationID": 5, "travelers": 1, "date": "2020/04/29", "duration": 16, "status": "approved", "suggestedActivities": []}, {"id": 163, "userID": 5, "destinationID": 48, "travelers": 1, "date": "2020/04/28", "duration": 10, "status": "approved", "suggestedActivities": []}])
    });
    ...
    it('travelerPastTrips function should return an empty array if there are no past trips from the log in date', () => {
      ...
      expect(travelerPastTrips(filteredTrips, date)).to.deep.equal([]);
    });
    ...
    it('travelerUpcomingTrips function should should filter the specific user trips data by the log in date and return all upcoming trips from that date', () => {
      const filteredTrips = getSpecificTravelerTrips(trips, id);
      expect(travelerUpcomingTrips(filteredTrips, date)).to.deep.equal([{"id": 196, "userID": 5, "destinationID": 16, "travelers": 1, "date": "2025/09/25", "duration": 8, "status": "pending", "suggestedActivities": []}]);
    });
    ...
    it('travelerUpcomingTrips function should return an empty array if there are no upcoming trips from the log in date', () => {
      ...
      expect(travelerUpcomingTrips(filteredTrips, date)).to.deep.equal([]);
    });
    ...
    it('travelerPendingTrips function should should filter the specific user trips data by the log in date and return all pending trips from that date', () => {
      ...
      expect(travelerPendingTrips(filteredTrips)).to.deep.equal([]);
    });

  it('travelerPendingTrips function should should return an empty array if there are no pending trips from that date', () => {
      const filteredTrips = getSpecificTravelerTrips(trips, id);
      expect(travelerPendingTrips(filteredTrips)).to.deep.equal([{"id": 196, "userID": 5, "destinationID": 16, "travelers": 1, "date": "2025/09/25", "duration": 8, "status": "pending", "suggestedActivities": []}]);
    });
  });
  ...
  describe('Calculate Cost functions:', () => {
    ...
    it('should create a function that filters trips by the year for that logged in user (filterTripByYear())', () => {
      const filteredTrips = getSpecificTravelerTrips(trips, id)
      expect(filterTripByYear(filteredTrips, year)).to.deep.equal([{ "id": 91, "userID": 5, "destinationID": 5, "travelers": 1, "date": "2020/04/29", "duration": 16, "status": "approved", "suggestedActivities": [] },
      { "id": 163, "userID": 5, "destinationID": 48, "travelers": 1, "date": "2020/04/28", "duration": 10, "status": "approved", "suggestedActivities": [] }
      ]);
    });
    ...
    it('this same function should return an empty [] if no year by that logged in user is found', () => {
      ...
      expect(filterTripByYear(filteredTrips,invalidYear)).to.deep.equal([]);
    });
    ...
    it('should create a function that calculates cost per year (getTotalCostPerYear())', () => {
      ...
      expect(getTotalCostPerYear(filterTripsByChosenYear,destinationData)).to.equal(3050);
    });
    ...
    it('should create a function that adds a 10% fee to the total cost', () => {
      ...
      expect(costWithFee(totalCostWithoutFee)).to.equal('3355.00');
    });
  });
  ...
  describe('Add New Trip functions:', () => {
    ...
    expect(calculateDuration(startDate, endDate, startDateInput, endDateInput)).to.equal(6)
    });
    ...
    it('calculateDuration function should return a duration of 1 if start date and end date are the same day', () => {
      ...
      const startDateInput = undefined;
      const endDateInput = undefined;
      expect(calculateDuration(startDate, endDate, startDateInput, endDateInput)).to.equal(1)
      });
    ...
    it('calculateDuration function should still work if the user inputs start date and end date', () => {
      ...
      expect(calculateDuration(startDate, endDate, startDateInput, endDateInput)).to.equal(1);
      expect(calculateDuration(startDate, endDate, startDateInput2, endDateInput2)).to.equal(5);
      });
    ...
    expect(costForNewTrip(tripObject, destinationData)).to.equal(1163)
    });
  });
});
